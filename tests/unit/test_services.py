...
from covid import InMemoryUnitOfWork
...
def test_can_add_user(in_memory_uow):
    ...
    auth_services.add_user(new_username, new_password, in_memory_uow)

    user_as_dict = auth_services.get_user(new_username, in_memory_uow)
...
def test_cannot_add_user_with_existing_name(in_memory_uow):
    ...
    with pytest.raises(auth_services.NameNotUniqueException):
        auth_services.add_user(username, password, in_memory_uow)
...
def test_authentication_with_valid_credentials(in_memory_uow):
    ...
    try:
        auth_services.authenticate_user(new_username, new_password, in_memory_uow)
...
def test_authentication_with_invalid_credentials(in_memory_uow):
    ...
    with pytest.raises(auth_services.AuthenticationException):
        auth_services.authenticate_user(new_username, '0987654321', in_memory_uow)
...
def test_can_add_comment(in_memory_uow):
    ...
    news_services.add_comment(article_id, comment_text, username, in_memory_uow)

    # Retrieve the comments for the article from the repository.
    comments_as_dict = news_services.get_comments_for_article(article_id, in_memory_uow)
...
def test_can_get_article(in_memory_uow):
    ...
    article_as_dict = news_services.get_article(article_id, in_memory_uow)
...
def test_cannot_get_article_with_non_existent_id(in_memory_uow):
    ...
    with pytest.raises(news_services.NonExistentArticleException):
        news_services.get_article(article_id, in_memory_uow)


def test_get_first_article(in_memory_uow):
    article_as_dict = news_services.get_first_article(in_memory_uow)

    assert article_as_dict['id'] == 1


def test_get_last_article(in_memory_uow):
    article_as_dict = news_services.get_last_article(in_memory_uow)
...
def test_get_articles_by_date_with_one_date(in_memory_uow):
    ...
    articles_as_dict, prev_date, next_date = news_services.get_articles_by_date(target_date, in_memory_uow)
...
def test_get_articles_by_date_with_multiple_dates(in_memory_uow):
...
def test_get_articles_by_date_with_non_existent_date(in_memory_uow):
...
def test_get_articles_by_id(in_memory_uow):
    target_article_ids = [5, 6, 7, 8]
    articles_as_dict = news_services.get_articles_by_id(target_article_ids, in_memory_uow)
...
def test_get_comments_for_article(in_memory_uow):
    comments_as_dict = news_services.get_comments_for_article(1, in_memory_uow)
...
def test_get_comments_for_non_existent_article(in_memory_uow):
    with pytest.raises(NonExistentArticleException):
        comments_as_dict = news_services.get_comments_for_article(7, in_memory_uow)


def test_get_comments_for_article_without_comments(in_memory_uow):
    comments_as_dict = news_services.get_comments_for_article(2, in_memory_uow)
